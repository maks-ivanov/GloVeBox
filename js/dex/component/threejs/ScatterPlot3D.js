...
var titleX = createText2D("-" + csv.header[0], new THREE.Color().setRGB(61, 89, 171), "", fontSize);
...
var titleY = createText2D('-' + csv.header[1], new THREE.Color().setRGB(61, 89, 171), "", fontSize);
...
var titleZ = createText2D('-' + csv.header[2], new THREE.Color().setRGB(61, 89, 171), "", fontSize);
...
function makeTextSprite( message, parameters )
{
    ...
    var fontsize = parameters.hasOwnProperty("fontsize") ? parameters["fontsize"] : fontSize*1.2;
...
for (i=0; i<csv.data.length; i++)
{
  ...
  { 
    var label = makeTextSprite(csv.labels[i]);
    label.position.x = csv.data[i][0] + fontOffset/3*(-xy*.7);
    label.position.y = csv.data[i][1] + fontOffset/2.5*xy;
    label.position.z = csv.data[i][j] + fontOffset/3*(-xy*.7);
